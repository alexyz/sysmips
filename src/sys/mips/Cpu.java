...
public final class Cpu {
	...
	private final CpuLogger logger;
	...
	public Cpu (Memory memory, Symbols symbols) {
		this.memory = memory;
		this.symbols = symbols;
		this.logger = new CpuLogger(symbols);
		...
		switch (op) {
			case OP_REGIMM:
				switch (rt) {
					case RT_BGEZAL:
						reg[31] = nextPc;
						System.out.println("RT_BGEZAL " + symbols.getName(nextPc));
						// fall through
					case RT_BGEZ:
						if (reg[rs] >= 0) {
							break;
					...
					case RT_BLTZAL:
						reg[31] = nextPc;
						System.out.println("RT_BLTZAL " + symbols.getName(nextPc));
						// fall through
					case RT_BLTZ:
						if (reg[rs] < 0) {
							break;
	...
	private final void execFn (int isn) {
		int rd = (isn >>> 11) & 0x1f;
		int rt = (isn >>> 16) & 0x1f;
		int rs = (isn >>> 21) & 0x1f;
		int fn = isn & 0x3f;
	...
	private void execFpuRs (int isn) {
		final int rs = rs(isn);
		final int rt = rt(isn);
		final int fs = fs(isn);
	...
	private void execFpuFn (final int isn, final Access access) {
		...
		switch (fn) {
			case FP_FN_ADD_D: {
				double fsValue = access.get(fpReg, fs);
				double ftValue = access.get(fpReg, ft);
	...
	private final void execCpFn (int isn) {
